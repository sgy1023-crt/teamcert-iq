...
- `lib/llm/llm-adapter.ts` —— 无依赖 OpenAI 兼容 + Azure OpenAI 客户端，9 秒超时，任何失败返回 null
- `app/api/assess/route.ts` —— POST，跑完整 7-agent 流程
- `app/api/status/route.ts` —— GET，返回 LLM 是否已配置（首页用）
...
### 高优先级（影响提交 / 演示）
1. **LLM 还没真跑过。** Manager Insight Agent 接好了 adapter，但本地没配 API key，所以首页和结果都显示 "Local fallback"。要让 demo 显示绿色 "LLM-assisted"，必须在 `.env.local` 配 `OPENAI_API_KEY` 或 Azure 那套，并实测一次 LLM 真的返回了四段叙事。**这是拿奖成色的关键，必须做。**
...
启用 LLM（可选，但强烈建议）—— 创建 `teamcert-iq-next/.env.local`：

```
# OpenAI 兼容（也支持代理 / vLLM / OpenRouter）
OPENAI_API_KEY=sk-...
...
不配任何 key 也能完整跑，只是 Manager Insight 走本地 fallback。
